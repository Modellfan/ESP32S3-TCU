...
.drop{border:1px dashed #8ea1ad;background:#f9fbfb;border-radius:8px;color:var(--muted);cursor:pointer}.drop.drag{border-color:var(--teal);background:#edf9f6;color:#146c5a}
  ...
  <main class="main">
    ...
    <section id="files" class="view">
      <div class="panel files-panel">
        ...
        <div class="file-grid drop" id="fileGrid" onclick="openFilePickerFromGrid(event)"></div>
      </div>
    </section>
...
function renderFiles(){
  ...
  $('fileGrid').innerHTML=cards||'<div class="empty-files">Drop files here or click to choose files</div>';
...
function openFilePickerFromGrid(event){
...
fileGrid.ondragover=e=>{e.preventDefault();fileGrid.classList.add('drag')};fileGrid.ondragleave=()=>fileGrid.classList.remove('drag');fileGrid.ondrop=async e=>{e.preventDefault();fileGrid.classList.remove('drag');for(let f of e.dataTransfer.files)await uploadFile(f,'/'+f.name);setTimeout(()=>listFiles(true),500)}
